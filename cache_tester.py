...
from history import load_ticker_history_raw, load_ticker_history_db, load_ticker_history_cached, dump_ticker_cache, \
...
if __name__ == '__main__':
    ...
    try:
        ...
        if module_config['write_cache']:
            ...
            # history = load_ticker_history_cached(ticker, module_config)
    except:
        traceback.print_exc()
    #
        # pass
    ...
    print(f"\nCompleted Cache test of {len(module_config['tickers'])} tickers in {int((int(time.time()) - start_time) / 60)} minutes and {int((int(time.time()) - start_time) % 60)} seconds")